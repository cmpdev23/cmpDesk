...
`AppLayout.tsx` reads `shouldUseDarkColors` from the main process on startup, then subscribes to `theme:changed` events so the `dark` class is always in sync:
...
Added Dark / Light / System theme switching via Electron's native menu bar ("Vue" menu).
...
| File | Change |
|------|--------|
| `electron/main.js` | Import `Menu`, `nativeTheme`; add `applyTheme()`, `buildMenu()`, IPC handlers |
| `electron/preload.js` | Add `themeAPI` (`getMode`, `setMode`, `onChange`); expose on `electronAPI.theme` |
| `src/types/electron.d.ts` | Add `ThemeAPI`, `ThemeMode`, `ThemeModeResult`, `ThemeSetResult` types |
| `src/layout/AppLayout.tsx` | Subscribe to theme changes; apply `dark` class dynamically |

### Rules

- **NEVER hardcode `dark` class** — Always use `isDark` state from theme IPC
- **Native menu is source of truth** — `nativeTheme.themeSource` drives everything
- **`system` mode** defers to OS preference — `shouldUseDarkColors` reflects the OS result
- **`themeAPI` is optional-chained** (`?.`) in renderer to avoid errors outside Electron
